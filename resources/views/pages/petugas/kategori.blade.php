<x-petugas-layout>
    ...
    <x-content>
        <x-card>
            ...
            <x-card-body>
                <x-input-search route="{{ route('petugas.kategori') }}">
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <div class="d-flex justify-content-center align-items-center">
                                        ...
                                        <x-button-delete id="{{ $data->id }}" resource="petugas/kategori"></x-button-delete>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
    ...
    @push('modal')
        <x-modal target="tambahKategori" title="Tambah Kategori" size="modal-lg">
            <form method="POST" action="{{ route('petugas.tambahKategori') }}" enctype="multipart/form-data" class="mb-2">
                @csrf
                <div class="row mb-2">
                    <div class="col-12">
    ...
    @push('script')
        ...
        <script>
            $('#editKategori').on('show.bs.modal', function(event) {
                ...
                $.get('/petugas/kategori/' + id + '/edit', function(data) {
                    var modal = $(this);
                    modal.find('#edit_nama').val(data.nama);

                    modal.find('form').attr('action', '/petugas/kategori/' + id + '/update');
                }.bind(this));
            });
        </script>
    @endpush
